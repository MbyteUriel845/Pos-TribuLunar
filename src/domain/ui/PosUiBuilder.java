...
import domain.ui.buttons.PriceButton;
import domain.ui.listeners.TextFieldFocusListener;
import domain.ui.listeners.CustomPriceKeyListener;
import domain.ui.listeners.DefaultKeyListener;
import domain.ui.listeners.PaymentKeyListener;
import domain.ui.models.ItemLineUiInfo;
import domain.ui.models.SaleUiInfo;
	...
	private static void addListeners (JFrame frame) {
		...
		priceButtons.forEach(button -> {
			button.addActionListener(actionEvent -> {
                String[] completo = button.getText().toString().split(" ",0);
                PosSaleController.getInstance().addItem( completo[2] , button.getPriceType().getPriceValue(), 1);
				updateSaleUiInfo();
			});
		});

		customPrice.addActionListener(actionEvent -> {
            String[] completo = customPrice.getText().toString().split(" ",0);
			PosSaleController.getInstance().addItemCustomPrice(completo[2] , customPrice.getText());
